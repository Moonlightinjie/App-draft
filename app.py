...
with tab1:
    ...
    with st.container():
        ...
        st.write("Enter your daily household energy usage to calculate the number of panels needed.")
    ...
    with st.container():
        ...
        if panel_size > 0 and roof_size > 0:
            cost_per_watt = st.slider("Estimated cost per watt (USD/W)", 0.5, 1.5, 1.0, help="Most solar panels cost between $0.80–$1.20 per watt.")
            custom_install = st.checkbox("I know my installation percentage")
            if custom_install:
                install_factor = st.slider("Enter installation and equipment cost (%)", 0, 100, 25, help="Includes inverter, wiring, mounting, and labor costs.") / 100
            else:
                install_factor = 0.25
            ...
            st.info("This estimate uses average solar prices. Installation includes inverter, wiring, and labor. Actual prices depend on your region and installer.")
        else:
